...
async def do_video(u,c,url):
    msg=await u.message.reply_text("⏳ Video yuklanmoqda...")
    d=ROOT/str(u.effective_user.id); d.mkdir(exist_ok=True)
    try:
        f,_=await asyncio.to_thread(video_download,url,d)
        await msg.edit_text("🎬 480p tayyorlanmoqda...\n📦 14 MB limit tekshirilmoqda...")
        f=await asyncio.to_thread(compress,f)
        await msg.edit_text(f"📤 Yuborilmoqda... {Path(f).stat().st_size/1024/1024:.2f} MB")
...
async def do_audio(u,c,url):
    msg=await u.message.reply_text("⏳ 128 kbps MP3 tayyorlanmoqda...")
    d=ROOT/str(u.effective_user.id); d.mkdir(exist_ok=True)
    try:
...
def search50(q):
    o=baseopts(); o.update({"extract_flat":True,"playlistend":50,"default_search":"ytsearch50"})
    with yt_dlp.YoutubeDL(o) as y:return [e for e in (y.extract_info("ytsearch50:"+q,download=False).get("entries") or []) if e][:50]

async def search(u,c):
    msg=await u.message.reply_text("🔎 50 ta variant qidirilmoqda...")
    try:
        rs=await asyncio.to_thread(search50,u.message.text.strip())
        c.user_data["results"]={str(i):e.get("webpage_url") or "https://www.youtube.com/watch?v="+e["id"] for i,e in enumerate(rs,1)}
        kb=[[InlineKeyboardButton(f"{i}. {(e.get('title') or 'Nomaʼlum')[:45]}",callback_data=f"s|{i}")] for i,e in enumerate(rs,1)]
        await msg.edit_text("🎵 50 ta natija. Keraklisini tanlang:",reply_markup=InlineKeyboardMarkup(kb))
    except Exception as e: await msg.edit_text("❌ Qidiruv xatosi:\n"+str(e)[-1800:])

async def buttons(u,c):
    q=u.callback_query; await q.answer()
    if q.data=="v": return await do_video(q,c,c.user_data.get("url"))
    if q.data=="a": return await do_audio(q,c,c.user_data.get("url"))
    if q.data.startswith("s|"):
        url=c.user_data.get("results",{}).get(q.data.split("|")[1])
        if not url:return await q.message.reply_text("❌ Natija eskirgan. Qayta qidiring.")
        return await do_audio(q,c,url)
